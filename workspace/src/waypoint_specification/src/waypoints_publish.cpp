...
public :
    Waypoints_publish() : move_base_action_("move_base", true), rate(10)
    {
        /* connect move_base */
        ...
        try {
            ...
            if(wp_node != NULL){
                for(int i=0; i < wp_node->size(); i++){
                    ...
                    (*wp_node)[i]["position"]["position_x"] >> pose.pose.position.x;
                    (*wp_node)[i]["position"]["position_y"] >> pose.pose.position.y;
                    (*wp_node)[i]["position"]["position_z"] >> pose.pose.position.z;
                    (*wp_node)[i]["position"]["orientation_x"] >> pose.pose.orientation.x;
                    (*wp_node)[i]["position"]["orientation_y"] >> pose.pose.orientation.y;
                    (*wp_node)[i]["position"]["orientation_z"] >> pose.pose.orientation.z;
                    (*wp_node)[i]["position"]["orientation_w"] >> pose.pose.orientation.w;
...
{
    ros::init(argc, argv, "waypoints_publish");
